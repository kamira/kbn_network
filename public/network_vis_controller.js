...
module.controller('KbnNetworkVisController', function ($scope, $sce, $timeout, Private) {
    var network_id, loading_id, legend_id;
    ...
    $scope.drawColorLegend = function (usedColors, colorDicc) {
        ...
        var textXPosition = canvas.width *  (-1 * $scope.vis.params.scalingXPosition)
        var textYPosition = canvas.height * (-1 * $scope.vis.params.scalingYPosition)
        var totalheight = usedColors.length * ($scope.vis.params.legendContentSize * 1.33) + ($scope.vis.params.legendTitleSize * 1.33) + ($scope.vis.params.legendPadding * 2) + 10


        context.fillStyle='rgba(255, 255, 255, 0.8)';
        context.filter="blur(4px)";
        context.fillRect(textXPosition - ($scope.vis.params.legendPadding), textYPosition - ($scope.vis.params.legendPadding), $scope.vis.params.legendWidth + ($scope.vis.params.legendPadding * 2), totalheight);


        context.filter="blur(0)";
        context.fillStyle = "black";
        context.font = `bold ${$scope.vis.params.legendTitleSize}px Arial`;
        context.textAlign = "start";
        context.fillText("LEGEND OF COLORS:", textXPosition, textYPosition);

        var p = textYPosition + ($scope.vis.params.legendTitleSize * 1.33) + 10;
        for (var key in colorDicc) {
            context.fillStyle = colorDicc[key];
            context.font = `bold ${$scope.vis.params.legendContentSize}px Arial`;
            context.fillText(key, textXPosition, p);
            p = p + ($scope.vis.params.legendContentSize * 1.33);
        }
    }
